...
!---------------------------------------------------------------
  ...
  ! (disabled: seems to produce numerical noise)
  ...
  do n=1,ne
     ...
     ! seems to prevent the numerical instability when finding delta
     ! that was observed with the previous choice (half the wavelength)
     lambda2 = pi/q/2.0_dp
     !
     rmin  = xmin
     ...
     do l=0,lmax
        ...
        delta = atan(tandelta)
        cross(l) = 4*pi/q**2 * (2*l+1)*sin(delta)**2
        !
