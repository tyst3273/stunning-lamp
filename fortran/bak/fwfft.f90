!
!----------------------------------------------------------------------
subroutine fwfft (nr1, nr2, nr3, psi)
  !----------------------------------------------------------------------
  !
  ! Forward f(G)=>f(r) Fourier transform of array psi, in-place
  ! Note that psi is in general complex and is here treated as real
  ! using the fortran trick: complex(1) == real(2)
  !
  implicit none
  integer, parameter :: dp = selected_real_kind(14,200)
  integer, intent(in):: nr1,nr2,nr3
  real(kind=dp), intent(inout) :: psi(2,nr1,nr2,nr3)

  call cft(psi(1,1,1,1),psi(2,1,1,1),nr1*nr2*nr3,nr1,nr1,        2)
  call cft(psi(1,1,1,1),psi(2,1,1,1),nr1*nr2*nr3,nr2,nr1*nr2,    2)
  call cft(psi(1,1,1,1),psi(2,1,1,1),nr1*nr2*nr3,nr3,nr1*nr2*nr3,2)

  return
end subroutine fwfft
!
!----------------------------------------------------------------------
subroutine invfft (nr1, nr2, nr3, psi)
  !----------------------------------------------------------------------
  !
  ! Inverse f(G)=>f(r) Fourier transform of array psi, in-place
  ! Note that psi is in general complex and is here treated as real
  ! using the fortran trick: complex(1) == real(2)
  !
  implicit none
